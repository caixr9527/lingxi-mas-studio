...
export function PlanPanel({ className }: PlanPanelProps) {
  ...
  return (
    <div className={cn("bg-white rounded-xl border", className)}>
      ...
      {!isExpanded && (
        <div
          className="flex flex-row items-start justify-between pr-3 relative clcikable cursor-pointer rounded-xl z-99"
          onClick={toggleExpand}
        >
          {/* 左侧最新计划 */}
          <div className="flex-1 min-w-0 relative overflow-hidden">
            <div className="w-full h-9">
              <div className="flex items-center justify-center gap-2.5 w-full px-4 py-2 truncate">
                <Clock size={16} />
                <div className="flex flex-col w-full gap-0.5 truncate">
                  <div className="text-sm truncate">
                    最新计划：完成用户登录功能开发，包含注册、登录、密码重置等模块
                  </div>
                </div>
              </div>
            </div>
                ...
                {steps.map((step) => (
                  ...
                  >
                    ...
                    {step.status === "completed" ? (
                      <Check
                        size={16}
                        className="relative t-0.5 flex-shrink-0"
                      />
                    ) : (
                      <Clock
                        size={16}
